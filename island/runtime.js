...
import {repairIslandStairs,stairAt,stairsRailBlocked} from '/67park-kimi-mobile/app/island-stair-geometry.js';
import {applyCurbJoins} from '/67park-kimi-mobile/app/island-curb-joins.js';
...
const GLB_DOSYASI=urlParams.get('model')==='pending'
  ...
  : '/67park-kimi-mobile/island/ada_calisma.glb';
...
await entryStage(13,'Finishing the northern neighbourhood');
  ...
  const parkEdgePatch=await islandFetch('/67park-kimi-mobile/repairs/park-edges-v4.json').then(r=>{if(!r.ok)throw Error('Park edge repair missing');return r.json();});
